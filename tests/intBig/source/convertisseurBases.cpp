...
QVector<quint64> convertisseurBases::CustomToCustom(QVector<quint64> value, quint64 base1, quint64 base2)//on prend des vecteurs qui ont ete créé avec les conversions QString en bas
{
    ...
    debug::d("CustomToCustom à "+QString::number(start));

    if(base1==base2)//si c les meme bases, on s'en occupe pas
    {
        quint64 stop=QDateTime::currentMSecsSinceEpoch();
        debug::d("CustomToCustom fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
        debug::stat("CustomToCustom", start, stop);
        return value;
    }
...
//les 2 fonctions suivantes sont plus utile
...
// ! modifs à continuer//   ^ peut etre mettre plus rapide que ++?
    ...
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("DecToCustom à "+QString::number(start));
    QVector<quint64> retour(1);
    retour[0]=0;
    if(isNull(value))//pas besoin de caluler plus
    {
        quint64 stop=QDateTime::currentMSecsSinceEpoch();
        debug::d("DecToCustom fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
        debug::stat("DecToCustom", start, stop);
        return retour;
    }
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("CustomToDec à "+QString::number(start));
    QVector<quint64> retour(1);
    retour[0]=0;
    if(isNull(value))//pas besoin de caluler plus
    {
        quint64 stop=QDateTime::currentMSecsSinceEpoch();
        debug::d("CustomToDec fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
        debug::stat("CustomToDec", start, stop);
        return retour;
    }
    if(baseCustom==10)//conversion10-10 etait déja bonne, petit raccourcis
    {
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("toVector(QString) à "+QString::number(start));
    QVector<quint64> retour(1);
    retour[0]=0;
    ...
    quint64 stop=QDateTime::currentMSecsSinceEpoch();
    debug::d("toVector(QString) fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
    debug::stat("toVector(QString)", start, stop);
    return retour;
}
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("toVector(quint64) à "+QString::number(start));
    ...
    quint64 stop=QDateTime::currentMSecsSinceEpoch();
    debug::d("toVector(quint64) fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
    debug::stat("toVector(quint64)", start, stop);
    return retour;
}
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("toString(QVector) à "+QString::number(start));
    QString retour;
    for(int i=0; i<value.size(); i++)
        retour[i] = QChar::unicodeVersion(value[i]);
    quint64 stop=QDateTime::currentMSecsSinceEpoch();
    debug::d("toString(QVector) fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
    debug::stat("toString(QVector)", start, stop);
    return retour;
}
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("pow à "+QString::number(start));
    QVector<quint64> retour(1);
    retour[0]=1;
    ...
    quint64 stop=QDateTime::currentMSecsSinceEpoch();
    debug::d("pow fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
    debug::stat("pow", start, stop);
    return retour;
}
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("estPetitOuEgal à "+QString::number(start));
    if(a.size() != b.size())//si ils ont une taille diff, regarder
    {
        quint64 stop=QDateTime::currentMSecsSinceEpoch();
        debug::d("estPetitOuEgal fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
        debug::stat("estPetitOuEgal", start, stop);
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("moins à "+QString::number(start));
    while(a.size() < b.size())
        a.append(0);
    while(a.size() > b.size())
        b.append(0);
    ...
    if(retenue)
    {
        debug::d("convertisseurBases::moins a est plus petit que b, il reste la retenue la valeur de retour est donc fausse", true);
        quint64 stop=QDateTime::currentMSecsSinceEpoch();
        debug::d("moins fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
        debug::stat("moins", start, stop);
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("plus à "+QString::number(start));
    while(a.size() < b.size())
        a.append(0);
    while(a.size() > b.size())
        b.append(0);
    ...
    while(a.last()==0 && a.size() > 1)//faudrait pas qu'il soit vide
        a.removeLast();
    quint64 stop=QDateTime::currentMSecsSinceEpoch();
    debug::d("plus fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
    debug::stat("plus", start, stop);
    return a;
}
...
{//multiplie les 2 vecteurs
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("fois à "+QString::number(start));
    QVector<quint64> retour(1);
    retour[0]=0;
    for(quint64 i=0; i<(quint64)a.size(); i++)
    {
    ...
    quint64 stop=QDateTime::currentMSecsSinceEpoch();
    debug::d("fois fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
    debug::stat("fois", start, stop);
    return retour;
}
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("estGrandOuEgal à "+QString::number(start));
    if(a.size() != b.size())//si ils ont une taille diff, regarder
    {
        quint64 stop=QDateTime::currentMSecsSinceEpoch();
        debug::d("estGrandOuEgal fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
        debug::stat("estGrandOuEgal", start, stop);
        return a.size() > b.size();
    }
