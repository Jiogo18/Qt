...
QString uintBig::toString(quint64 base) const
{
    ...
    debug::d("toString fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
    debug::stat("toString", start, stop);
    return retour;
}
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("fromText à "+QString::number(start));
    ...
    quint64 stop=QDateTime::currentMSecsSinceEpoch();
    debug::d("fromText fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
    debug::stat("fromText", start, stop);
    return retour;
}
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("!= à "+QString::number(start));
    if(size() != v.size())//normalement impossible
    {
        quint64 stop=QDateTime::currentMSecsSinceEpoch();
        debug::d("!= fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
        debug::stat("!=", start, stop);
        return true;
    }
...
void uintBig::operator +=(const quint64 &v) {operator =(operator +(v));}
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("-quint64 à "+QString::number(start));
    if(negatif)
    {//si je suis negatif
        uintBig t(valeur, baseDesCases);//     - "-" +
        t.setNegative(!negatif);        //=> -(+ "+" -)
        ...
        t.setNegative(!t.isNegative());
        quint64 stop=QDateTime::currentMSecsSinceEpoch();
        debug::d("-quint64 fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
        debug::stat("-quint64", start, stop);
        return t;
    }
    uintBig retour(valeur, baseDesCases);
    quint64 retenue=0;
...
{//division
    ...
    if(v.isEmpty())
    {
        debug::d("erreur division, le dénominateur vaut 0 "+v.toString(), true);
        quint64 stop=QDateTime::currentMSecsSinceEpoch();
        debug::d("/ fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
        debug::stat("/", start, stop);
        return retour;
...
uintBig uintBig::operator /(quint64 v) const
{
    ...
    uintBig retour2(retour, v);//on le repasse en uintBig
...
uintBig uintBig::operator %(uintBig v)
{
    ...
    resize();
    v.resize();
    //debug()<<toString()<<"%"<<v.toString()<<"ou"<<toVector()<<"%"<<v.toVector();
    ...
    retour.resize();
    //debug()<<"="<<retour.toString()<<"ou"<<retour.toVector();
    quint64 stop=QDateTime::currentMSecsSinceEpoch();
    debug::d("%% fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
    debug::stat("%", start, stop);
    return retour;
...
{
    quint64 start=QDateTime::currentMSecsSinceEpoch();
    debug::d("sqrt à "+QString::number(start));
    resize();
    if(isNegative())
    {
        debug::d("erreur sqrt, racine d'un nombre négatif", true);
        quint64 stop=QDateTime::currentMSecsSinceEpoch();
        debug::d("sqrt fin de "+QString::number(start)+", durée: "+QString::number(stop-start)+" msec");
        debug::stat("sqrt", start, stop);
        return 0;
    }
    if(isEmpty())
    {
...
void uintBig::resize()
{
    while(valeur.size()<64)//pas bien cadré
        valeur.append(0);
    if(valeur.size()>64)
        debug::d("un unintBig fait plus de 64 cases ("+QString::number(valeur.size())+")", true);
